...
export global_confluence_backup_dir="/var/backups/confluence-backups"
export daily_backup_dir="$global_confluence_backup_dir/daily"
export weekly_backup_dir="$global_confluence_backup_dir/weekly"
export monthly_backup_dir="$global_confluence_backup_dir/monthly"
export log_dir="$global_confluence_backup_dir/log"
export specific_backup_dir="$global_confluence_backup_dir/$date_regex"

main() {
        create_key_for_encrypt
        create_specific_backup_dir_and_log_dir
        create_timely_backup_dir
        get_backup_from_confluence_files
...
get_backup_from_confluence_files() {
        cp -r /root/confluence/ "$specific_backup_dir/"
}

tar_specific_backup_dir_then_encrypt() {
        tar -czvf "$global_confluence_backup_dir/$date_regex-confluence.tar.gz" $specific_backup_dir
        pushd $global_confluence_backup_dir/
        ccrypt -e -k $key_of_ccryptkey "$date_regex-confluence.tar.gz"
        popd
}
...
move_tar_backup_file_to_timely_dirs() {
        cp "$global_confluence_backup_dir/$date_regex-confluence.tar.gz.cpt" $daily_backup_dir/
        if [[ $(date +%u) -eq 6 ]]; then
                cp "$global_confluence_backup_dir/$date_regex-confluence.tar.gz.cpt" $weekly_backup_dir/
        fi
        if [[ $(date +%d) -eq 29 ]]; then
                cp "$global_confluence_backup_dir/$date_regex-confluence.tar.gz.cpt" $monthly_backup_dir/
        fi
}

remove_first_tar_backup_file() {
        rm -rf "$global_confluence_backup_dir/$date_regex-confluence.tar.gz.cpt"
}

send_daily_file_to_sftp_server() {
        sudo sshpass -p 'your_sftp_password' sftp your_sftp_server:your_sftp_location <<< "put $daily_backup_dir/$date_regex-confluence.tar.gz.cpt"
}

send_weekly_file_to_sftp_server() {
        if [[ $(date +%u) -eq 6 ]]; then
                sudo sshpass -p 'your_sftp_password' sftp your_sftp_server:your_sftp_location <<< "put $weekly_backup_dir/$date_regex-confluence.tar.gz.cpt"
        fi
}

send_monthly_file_to_sftp_server() {
        if [[ $(date +%d) -eq 29 ]]; then
                sudo sshpass -p 'your_sftp_password' sftp your_sftp_server:your_sftp_location <<< "put $monthly_backup_dir/$date_regex-confluence.tar.gz.cpt"
        fi
}
...
check_backup_file_size_and_send_notification_mail() {
        minimumsize=50000
        actualsize=$(du -k "$daily_backup_dir/$date_regex-confluence.tar.gz.cpt" | cut -f 1)
        if [ $actualsize -ge $minimumsize ]; then
                echo "about confluence ; backup was successful ; size of backup file is : $(echo $actualsize ) kilobytes ; name of backup file is : $(echo $date_regex-confluence.tar.gz.cpt)" | mail -s "confluence backup" your_email_address
        else
                echo "about confluence ; backup was not successful ; size of backup file is : $(echo $actualsize ) kilobytes " | mail -s "confluence backup" your_email_address
        fi
}
